...
import csv
import numpy as np
from adviceFileS import adviceSet
import json
...
def parseEntities(f):
    ...
    cols = []
    data = []    
    with open('../data/social/socialmedia-disaster-tweets-DFEshortened.csv', 'rU') as f:
        line = csv.reader(f)
        count =0
        for row in line:
            #print row[len(row)-1]
                ...
                for k in entitiesInQuestion:
                    if not str.startswith(k,"?"):
                        if (hasWordinEntity(ent, entitiesInQuestion[k], k) is True) and (entitiesInQuestionCon[k] is not None) and ((k in rel_list[targetEnt]) or (targetEnt in rel_list[k])):
                            rel = targetEnt
                            if entitiesInQuestionCon[k] is 'sameUser':
...
def getNeighborList(entity, rel_list, relname):
    ...
    if entity > len(entity_list):
        ...
        if relname is 'sameUser':
            newL = rel_list[entity][0]
        if relname is 'sameLoc':
