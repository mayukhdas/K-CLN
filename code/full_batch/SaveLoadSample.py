...
def save_sample(train_ids,valid_ids,test_ids, name, loc):
    ...
    train_ids_sample = random.sample(train_ids, int(len(train_ids)*0.8))
    valid_ids_sample = random.sample(valid_ids, int(len(valid_ids)*0.8))
...
#save_sample(train_ids, valid_ids, test_ids, 'debateSample80P', '../data/')
