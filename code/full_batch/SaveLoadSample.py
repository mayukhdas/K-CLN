...
def save_sample(train_ids,valid_ids,test_ids, name, loc):
    ...
    train_ids_sample = random.sample(train_ids, int(len(train_ids)*0.05))
    valid_ids_sample = random.sample(valid_ids, int(len(valid_ids)*0.05))
    test_ids_sample = random.sample(test_ids, int(len(test_ids)*0.05))
...
#save_sample(train_ids, valid_ids, test_ids, 'pubmedSample5P.json', '../data/')
