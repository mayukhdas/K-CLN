...
def create_resNet(n_layers, hidden_dim, input_dim, n_rel, n_neigh, n_classes, shared, nmean, dropout=True):
    ...
    # print("Nodes ", type(inp_nodes))
    # # print("Inp_rel: ", K.eval(inp_rel).shape)
    # print("Nodes ",K.eval(inp_nodes).shape)
    # print("Inp_rel: ",K.eval(inp_rel).shape)
    # print("Inp_rel_mask",K.eval(inp_rel_mask).shape)
...
def create_dense(n_layers, hidden_dim, input_dim, adv_dim, n_rel, n_neigh, n_classes, shared, nmean=1, dropout=True):
    ...
    tempTop = Input(shape=(n_classes), dtype='float32', name='inp_c_adv_mask')
    # print("Nodes ", inp_nodes.shape)
    # print("Inp_rel: ",inp_rel.eval())
    # print("Inp_rel_mask",type(inp_rel_mask))
    # print("Inp_rel: ", K.eval(inp_rel).shape)
    # print("Nodes ",inp_nodes.get_value())
    # print("Inp_rel: ",K.eval(inp_rel).shape)
    # print("Inp_rel_mask",K.eval(inp_rel_mask).shape)

    hidd_nodes = Dense(input_dim=input_dim, output_dim=hidden_dim, init=init, activation=act)(inp_nodes)

    # print("hidden_nodes: ", type(hidd_nodes))
    if dropout: hidd_nodes = Dropout(0.5)(hidd_nodes)
    ...
    top_nodes = Dense(output_dim=n_classes, input_dim=hidden_dim)(hidd_nodes)
    top_nodes = Activation(activation=top_act)(top_nodes)
    model = Model(input=[inp_nodes, inp_rel, inp_rel_mask, inp_I_adv, inp_W_adv_mask, inp_c_adv_mask, None], output=[top_nodes]) # changes by MD & Yang
